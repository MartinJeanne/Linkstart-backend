...
public class MemberService {
    ...
    public ResponseEntity<CollectionModel<MemberDto>> getMembers(String discordId) {
        List<Member> members;
        if (discordId != null && !discordId.trim().isEmpty()) {
            Member member = memberRepo.findByDiscordId(discordId).orElseThrow(NoContentException::new);
            members = new ArrayList<>();
            members.add(member);
        } else members = memberRepo.findAll();
